    ...
    async function addNote(e) {
      ...
      if (!res.ok) { 
        alert("❌ Failed to add note: " + (data && data.error) || "Unknown error"); 
        return; 
      }
    ...
    async function load() {
      ...
      list.forEach(function (n) {
        ...
        if (n.mentions && n.mentions.length > 0) {
          mentionsInfo = `<span class="meta">👥 Mentioned: ${n.mentions.map(m => '@' + m).join(', ')}</span>`;
        }
    ...
    if (currentUserId) {
      q("#author_id").value = currentUserId;
      q("#loginStatus").textContent = "✓ Logged in";
      q("#loginStatus").className = "logged-in";
    } else {
      q("#loginStatus").textContent = "⚠ Not logged in";
