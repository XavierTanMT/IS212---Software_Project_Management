...
class TestCreateUser:
    ...
    def test_create_user_with_role(self, client, mock_db, monkeypatch):
    ...
    def test_create_user_invalid_role(self, client, mock_db, monkeypatch):
    ...
    def test_create_user_default_role(self, client, mock_db, monkeypatch):
        """Test creating a user without specifying role (should default to staff)"""
        mock_ref = Mock()
        mock_get_result = Mock()
        mock_get_result.exists = False
        mock_ref.get.return_value = mock_get_result
...
class TestEdgeCases:
    ...
    def test_get_user_role_success(self, client, mock_db, monkeypatch):
        """Test successfully retrieving a user's role"""
        mock_doc = Mock()
        mock_doc.exists = True
        mock_doc.to_dict.return_value = {
            "user_id": "user123",
        ...
        response = client.get("/api/users/user123/role")
    ...
    def test_get_user_role_not_found(self, client, mock_db, monkeypatch):
        """Test error when user doesn't exist for role endpoint"""
        mock_doc = Mock()
        mock_doc.exists = False
        ...
        response = client.get("/api/users/nonexistent/role")
    ...
    def test_get_user_role_defaults_to_staff(self, client, mock_db, monkeypatch):
        """Test that role defaults to staff when not set"""
        mock_doc = Mock()
        mock_doc.exists = True
        mock_doc.to_dict.return_value = {
            "user_id": "user123",
